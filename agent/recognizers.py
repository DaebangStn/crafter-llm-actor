...
def _pattern_recognizer(_window: dict, reference_image_path: str) -> list:
...
def _inventory_recognizer(_window: np.ndarray) -> dict:
    assert isinstance(_window, np.ndarray), "window must be a numpy array"
...
def _recognized_tag_writer(_window: np.ndarray, recognizer_out: dict) -> np.ndarray:
    assert isinstance(_window, np.ndarray), "window must be a numpy array"
...
def surrounding_recognizer(
        _window: np.ndarray, reference_image_dir_path: str, display=False
) -> (Dict[str, list], bool):
    assert isinstance(_window, np.ndarray), "window must be a numpy array"
    assert os.path.isdir(reference_image_dir_path), f"ref_img_path: {reference_image_dir_path} must be a valid directory path"

    # convert BGR to RGB
    _window = cv2.cvtColor(_window, cv2.COLOR_BGR2RGB)
    ...
    for reference_image in reference_images:
        loc = _pattern_recognizer(_window, reference_image)
    ...
    ambient_rel = _convert_absolute_ambient_to_relative(ambient_abs)
    # print("ambient relative position: ", ambient_rel)

    isNight = _night_detector(ambient_rel)
    # ambient_rel["isNight"] = isNight
    # print("isNight: ", isNight)

    # window_status = inventory_recognizer(_window)
    # ambient_rel["inventory"] = window_status
    # print("window status: ", window_status)

    if display:
        _window = _recognized_tag_writer(_window, ambient_abs)
        # Display the image
        cv2.imshow("Detected Position", _window)
        cv2.imwrite("Detected Position.jpg", _window)

    return ambient_rel, isNight


# divide the coordinate by 66 to get the tile position
# subtract (4, 3) to get the relative position
# make the coordinate unique
def _convert_absolute_ambient_to_relative(ambient_abs: dict) -> dict:
    ambient_rel = {}
...
def _night_detector(ambient_rel: dict) -> bool:
...
class PlayerOrientation:
    ...
    def update_action(self, action: str):
        if 'move' in action:
            self._orientation = action.split('_')[1]
